...
def print_version():
    ...
    find_java_binary()
    check_basic_env()
    logger.info('print_version finished')
...
def check_basic_env():
    ...
    logger.info('check_basic_env：import http_tools')
    try:
        import lxml  # noqa F401
    except ImportError:
        logger.exception('lxml is not installed!')
        os.kill(os.getpid(), signal.SIGTERM)
    logger.info('check_basic_env：import lxml')
    if not is_file_exists(find_java_binary()):
        ...
        os.kill(os.getpid(), signal.SIGTERM)
    logger.info('check_basic_env：import find_java_binary')
    # get_adb()
